...
def linkify_insurance_html(text):
    ...
    if " – " in text:
        ...
        if find_insurance_url(insurance):
            return f"{rating} – <span class='insurance-link'>{insurance}</span>"
        return text
    if find_insurance_url(text):
        return f"<span class='insurance-link'>{text}</span>"
    return text
...
def credit_rank(text):
    ...
    if "R-1 (HIGH)" in value or "AA (HIGH)" in value: return 100
    if "R-1 (MID)" in value or "AA" in value:          return 95
    if "R-1 (LOW)" in value or "AA (LOW)" in value:    return 90
    if "R-2 (HIGH)" in value:                          return 80
    if "R-2" in value:                                 return 75
    if "BBB (HIGH)" in value:                          return 70
    if "BBB" in value:                                 return 65
    if "100% GUARANTEE" in value:                      return 60
    if "$250,000" in value:                            return 50
    if "$125,000" in value:                            return 45
    if "$100,000" in value:                            return 40
    return 0
...
def rating_and_insurance(raw_name, term_type, lookup):
    ...
    if not info:
        return ""
    rating = info["long_rating"] if term_type == "long" else info["short_rating"]
...
def keep_best_per_institution(rows):
    ...
    for row in rows:
        ...
        if row[3] > existing[3]:
            best[key] = row
        elif row[3] == existing[3] and credit_rank(row[1]) > credit_rank(existing[1]):
            best[key] = row
...
def generate_custom_query(master_file, lookup, selected_terms, top_n, credit_rated_only):
    ...
    for display_term in selected_terms:
        ...
        for _, row in df_master.iterrows():
            if clean_text(row.get("available")).lower() != "available":
                continue
...
def _merge_term_cells(ws):
    start_row = 2
    current_term = ws.cell(start_row, 3).value
    max_row = ws.max_row
    for row in range(3, max_row + 2):
        next_term = ws.cell(row, 3).value if row <= max_row else None
...
def create_excel(output):
    ...
    ws.append(["Issuer", "Credit Rating & Guarantee", "Term", "Rate"])
